...
def test_resolve_exp_known():
    ...
    assert cli._resolve_exp("grpo_qwen3.5-9b_gsm8k_v1") == "experiments/grpo_qwen3.5-9b_gsm8k_v1"
...
def test_list_exps_nonempty():
    exps = cli._list_exps()
    assert "grpo_qwen3.5-9b_gsm8k_v1" in exps
...
def test_validate_exp_clean_on_real_experiment():
    errors, _ = cli._validate_exp("experiments/grpo_qwen3.5-9b_gsm8k_v1")
    assert errors == []
